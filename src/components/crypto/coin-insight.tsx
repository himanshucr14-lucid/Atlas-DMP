    ...
    <Card className="lg:col-span-8">
      <CardHeader>
        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
          <CardTitle>{coin?.name ?? "RTB Exchange"} Clearing CPM Curve</CardTitle>
          <span className={cn(
            "rounded px-2 py-0.5 text-sm font-bold tabular-nums font-mono",
            lastIsGrowing ? "text-emerald-500" : "text-rose-500"
          )}>
            ${livePrice.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} eCPM
          </span>
        </div>
        <CardAction>
          <Button variant="ghost" size="sm" className="gap-1 text-xs text-muted-foreground">
            Exchange Directives <ArrowRightIcon className="size-3" />
          </Button>
        </CardAction>
      </CardHeader>
      <CardContent className="min-w-0 space-y-4">
        ...
        <div className="flex items-center gap-4 text-xs text-muted-foreground">
          <span className="flex items-center gap-1.5">
            <span className="inline-block size-2.5 rounded-sm bg-emerald-500" /> High Yield (CPM UP)
          </span>
          <span className="flex items-center gap-1.5">
            <span className="inline-block size-2.5 rounded-sm bg-rose-500" /> Floor Risk (CPM DOWN)
          </span>
        </div>
