...
export function MyBalance({ prices }: { prices: CryptoPrices }) {
  ...
  const stats = [
    ...
    { label: "Win Rate", value: "78.4%", positive: true },
    {
      label: coin ? coin.name : "Primary Exchange",
      value: coin
        ? `$${coinPrice.toFixed(2)} eCPM`
        : "Rewarded Video",
      positive: null,
    },
  ]
  ...
  function handleTopUp() {
    ...
    if (topUpFlow === "input") {
      ...
      setTimeout(() => {
        setSuccessMsg(`Re-indexed Floor Price to $${val.toFixed(2)}`)
  ...
  function handleWithdraw() {
    ...
    if (withdrawFlow === "input") {
      ...
      setTimeout(() => {
        setSuccessMsg(`Flushed ${val.toFixed(0)} queued bids`)
  ...
  return (
    <Card className="lg:col-span-4">
      <CardHeader>
        <CardTitle>DSP Auction Liquidity</CardTitle>
        <CardAction>
      ...
      <CardContent className="space-y-4">
        <motion.p
          ...
          className="text-3xl font-bold tabular-nums tracking-tight font-mono"
        >
          ${totalBalance.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          <span className="text-xs text-muted-foreground font-normal ml-2">AVG CPM</span>
        </motion.p>
        ...
        <div className="flex gap-2 pt-1">
          ...
          >
            ...
            {topUpFlow === "input" ? "Confirm" : topUpFlow === "loading" ? "Updating..." : "Set Floor CPM"}
          </Button>
          <Button
            variant="outline"
            className="flex-1 gap-2 text-xs"
          ...
          >
            ...
            {withdrawFlow === "input" ? "Confirm" : withdrawFlow === "loading" ? "Flushing..." : "Flush Queue"}
          </Button>
        </div>
      </CardContent>
    </Card>
